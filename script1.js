const btn = document.getElementById('btnOne');
    const remain = document.getElementsByClassName('count');
    ...
    btn.addEventListener('click', () => {
        ...
        remain[0].value = remainSpace;
        remain[1].value = filledSpace;
    })


//Entry Class: Represent each entry in the parking lot
class Entry{
    constructor(spot,owner,car,licensePlate,entryDate,exitDate,entryTime,exitTime){
        this.spot=spot;
        this.owner = owner;
        this.car = car;
...
class UI{
    ...
    static addEntryToTable(entry){
        ...
        row.innerHTML = `  <td>${entry.spot}</td>
                            <td>${entry.owner}</td>
                            <td>${entry.car}</td>
    ...
    static validateInputs(){
        ...
        if(spot===''||owner === '' || car === '' || licensePlate === '' || entryDate === '' || exitDate === ''){
...
//Event Add
    document.querySelector('#entryForm').addEventListener('submit',(e)=>{
        ...
        //Instatiate Entry
        const entry = new Entry(spot,owner, car, licensePlate, entryDate, exitDate, entryTime, exitTime);
